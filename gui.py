...
from main import *
from image_alter import get_color_map, binarize, change_aspect_ratio
import saved_models
...
def create_buttons():
    ...
    def read():
        map = [[1.0  if cells_map[i][j].is_activated else 0.0 for i in range(len(cells_map))] for j in range(len(cells_map))]
        Tmap = torch.tensor(map)
        print(torch.argmax(MODEL(Tmap)), MODEL(Tmap))

    def test_import():
        img = Image.open("images_from_phone/siffra_3.png")
        color_map = change_aspect_ratio(binarize(img, 50), 28, 28)
        
        print(color_map)
        r = Tk()
        L = Label(r, image=color_map).pack()
        r.mainloop()
        print(img)
        for i in range(28):
            for j in range(28):
    ...
    def load():
        global MODEL
        MODEL = load_model(CNN, "one_layer_10_neurons.pt", 10)
        load_button.configure(highlightbackground=colors["white"])
        print("D O N E")
    ...
    load_button = Button(canvas, text="LOAD MODEL", highlightbackground=colors["rich_black"], command=load)
    load_button.place(x=500, y=button_y)
create_buttons()
