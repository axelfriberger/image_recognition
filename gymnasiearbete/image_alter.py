...
images_1_5 = [img2.crop([int(round(i*w/5)), 0, int(round((i+1)*w/5)), int(round(h/2-20))]) for i in range(5)] 

new_img_1_5 = [ImageTk.PhotoImage(image) for image in images_1_5]

for i in range(len(new_img_1_5)):
    #label = Label(canvas, image = new_img_1_5[i]).pack(side=LEFT)
    pass
...
if __name__ == "__main__":
    ...
    print(get_color_map(small_one))
    image_new = ImageTk.PhotoImage(small_one)
    label1 = Label(canvas, image = image_new)
    label1.pack()
